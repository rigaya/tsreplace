...
先頭・中間・末尾のいずれのカットも、```--cut-list``` のカットリスト内で指定します。

| カット位置 | カットリストでの指定 | タイムスタンプの詰め直し |
|---|---|---|
| 先頭のカット | ```cut -1 <pts>``` | しない (出力開始点を決めるのみ) |
| 中間の途中区間カット | ```cut <start> <end>``` | する |
| 末尾のカット | ```cut <pts> -1``` | しない (出力終了点を決めるのみ) |
