...
インタレ保持の場合はx264を使用します。

#### x264

  `ffmpeg.exe -y -i <入力tsファイル> -copyts -start_at_zero -an -c:v libx264 -flags +ildct -preset slow -crf 23 -g 90 -f mpegts - | tsreplace.exe -i <入力tsファイル> -r - -o <出力tsファイル>`
